	...
	Context("Get valid supplier", func() {
		Context("PaymentAccountDetails Doen't Have Warehouses Mapped", func() {
			It("Should Respond with success", func() {
				...
				Expect(resp.Data.PartnerServiceMappings).To(HaveLen(2))
				Expect(resp.Data.PartnerServiceMappings[0].ServiceType).To(Equal("Supplier"))
				Expect(resp.Data.PartnerServiceMappings[0].ServiceLevel).To(Equal("Hlc"))
				Expect(resp.Data.PartnerServiceMappings[0].Active).To(Equal(true))
				Expect(resp.Data.PartnerServiceMappings[1].ServiceType).To(Equal("Transporter"))
				Expect(resp.Data.PartnerServiceMappings[1].ServiceLevel).To(Equal("Driver"))
				Expect(resp.Data.PartnerServiceMappings[1].Active).To(Equal(false))
